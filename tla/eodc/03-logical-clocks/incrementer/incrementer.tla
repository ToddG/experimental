...
macro debug(name) begin
    if Debug then
        ...
        print("proc_history: " \o ToString(proc_history));
    end if;
end macro;
    ...
    begin
        ProcedureLabel1:
            ...
            if x < MaxValue then
                x := x + 1;
                proc_values[proc] := x;
                proc_history[proc] := Append(proc_history[proc], x);
            end if;
...
VARIABLES proc_values, proc_history, pc, stack
...
vars == << proc_values, proc_history, pc, stack, x, proc, 
           procedure_local_y_not_used, proc_local_var_a >>
...
ProcedureLabel1(self) == /\ pc[self] = "ProcedureLabel1"
                         /\ IF Debug
                               THEN /\ PrintT(("----"))
                                    /\ PrintT(("Name: " \o ToString("ProcedureLabel1")))
                                    /\ PrintT(("Self: " \o ToString(self)))
                                    /\ PrintT(("Procs: " \o ToString(Procs)))
                                    /\ PrintT(("proc_values: " \o ToString(proc_values)))
                                    /\ PrintT(("proc_history: " \o ToString(proc_history)))
                               ELSE /\ TRUE
                         /\ IF x[self] < MaxValue
                               THEN /\ x' = [x EXCEPT ![self] = x[self] + 1]
                                    /\ proc_values' = [proc_values EXCEPT ![proc[self]] = x'[self]]
                                    /\ proc_history' = [proc_history EXCEPT ![proc[self]] = Append(proc_history[proc[self]], x'[self])]
                               ELSE /\ TRUE
                                    /\ UNCHANGED << proc_values, proc_history, 
                                                    x >>
